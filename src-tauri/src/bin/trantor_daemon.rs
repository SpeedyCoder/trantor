...
#[path = "../storage.rs"]
mod storage;
#[path = "trantor_daemon/transport.rs"]
mod transport;
#[allow(dead_code)]
#[path = "../types.rs"]
mod types;
...
const DAEMON_NAME: &str = "trantor-daemon";
...
fn default_data_dir() -> PathBuf {
    if let Ok(xdg) = env::var("XDG_DATA_HOME") {
        ...
        if !trimmed.is_empty() {
            return PathBuf::from(trimmed).join("trantor-daemon");
        }
    }
...
fn usage() -> String {
    format!(
        "\
USAGE:\n  trantor-daemon [--listen <addr>] [--data-dir <path>] [--token <token> | --insecure-no-auth]\n\n\
...
mod tests {
    ...
    fn test_state(data_dir: &std::path::Path) -> DaemonState {
        ...
        DaemonState {
            ...
            codex_login_cancels: Mutex::new(HashMap::new()),
            daemon_binary_path: Some("/tmp/trantor-daemon".to_string()),
        }
    }
...
fn main() {
    ...
    runtime.block_on(async move {
        ...
        eprintln!(
            "trantor-daemon listening on {} (data dir: {})",
            config.listen,
            state
                .storage_path
                .parent()
