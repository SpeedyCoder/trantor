...
Canonical navigation guide for Trantor. Use this as: "if you need X, edit Y".
...
6. Daemon RPC method parity: `src-tauri/src/bin/trantor_daemon/rpc.rs`
7. Daemon state/wiring implementation: `src-tauri/src/bin/trantor_daemon.rs`
8. Standalone daemon lifecycle CLI: `src-tauri/src/bin/trantor_daemonctl.rs`
...
| Change Git/GitHub backend behavior | `src/features/git/hooks/*`, `src/services/tauri.ts`, `src-tauri/src/git/mod.rs`, `src-tauri/src/shared/git_ui_core.rs`, `src-tauri/src/shared/git_ui_core/*`, `src-tauri/src/shared/git_core.rs`, `src-tauri/src/bin/trantor_daemon/rpc.rs`, `src-tauri/src/bin/trantor_daemon/rpc/git.rs` |
| Change prompts CRUD/listing behavior | `src/features/prompts/hooks/useCustomPrompts.ts`, `src/features/prompts/components/PromptPanel.tsx`, `src/services/tauri.ts`, `src-tauri/src/prompts.rs`, `src-tauri/src/shared/prompts_core.rs`, `src-tauri/src/bin/trantor_daemon/rpc.rs` |
| Change file read/write for Agents/config | `src/services/tauri.ts`, `src-tauri/src/files/mod.rs`, `src-tauri/src/shared/files_core.rs`, `src-tauri/src/bin/trantor_daemon/rpc.rs` |
| Add/change daemon JSON-RPC surface | `src-tauri/src/bin/trantor_daemon/rpc.rs`, `src-tauri/src/bin/trantor_daemon/rpc/*`, `src-tauri/src/bin/trantor_daemon.rs`, matching shared core |
...
- Daemon entrypoint and state/wiring: `src-tauri/src/bin/trantor_daemon.rs`
- Daemon lifecycle CLI (headless start/stop/status): `src-tauri/src/bin/trantor_daemonctl.rs`
- Daemon JSON-RPC dispatcher/router: `src-tauri/src/bin/trantor_daemon/rpc.rs`
- Daemon domain handlers: `src-tauri/src/bin/trantor_daemon/rpc/*`
- Daemon transport: `src-tauri/src/bin/trantor_daemon/transport.rs`
