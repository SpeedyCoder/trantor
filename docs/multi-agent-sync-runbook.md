...
Keep Trantor's Agents settings behavior in sync with upstream Codex (`../Codex`) whenever Codex changes:
...
- When users report a mismatch between Trantor settings and Codex runtime behavior
...
## Trantor Files To Update If Upstream Changes
...
- `src-tauri/src/bin/trantor_daemon/rpc/codex.rs`
...
- Keep Trantor scoped to the new key only (no legacy alias read/write).
- Keep Trantor writes aligned with upstream expectations.
...
- Reconcile Trantor guardrails when upstream changes.
...
- Trantor default `agents.max_depth` is `1`.
- Trantor currently enforces a product cap of `12` for `agents.max_threads` and `4` for `agents.max_depth` in UI + backend.
