...
- Which app-server events Trantor supports right now.
- Which app-server requests Trantor sends right now.
- Where to look in Trantor to add support.
...
3. Compare Codex events vs Trantor routing.
4. Compare Codex client request methods vs Trantor outgoing request methods.
5. Compare Codex server request methods vs Trantor inbound request handling.
6. Update supported and missing lists below.

Related project skill:
- `.codex/skills/app-server-events-sync/SKILL.md`

## Where To Look In Trantor
...
- `src-tauri/src/bin/trantor_daemon.rs`

## Supported Notifications (Codex v2)

These are the current Codex v2 `ServerNotification` methods that Trantor
...
## Additional Stream Methods Handled In Trantor
...
- `codex/backgroundThread` (Trantor synthetic bridge event)
- `codex/connected` (Trantor synthetic bridge event)
...
## Supported Requests (Trantor -> App-Server, v2)

These are v2 request methods Trantor currently sends to Codex app-server:
...
Compared against Codex v2 request methods, Trantor currently does not send:
...
## Server Requests (App-Server -> Trantor, v2)
...
4. List Trantor outgoing requests:
...
6. Check for camelCase vs snake_case mismatches:
   ...
   - Trantor generally defends against this by checking both forms (for example in `threadNormalize.ts` and `useAppServerEvents.ts`), while centralizing method/type parsing in `appServerEvents.ts`.
...
- Steering behavior while a turn is processing:
  - Trantor attempts `turn/steer` only when steer capability is enabled, the thread is processing, and an active turn id exists.
  - If `turn/steer` fails, Trantor does not fall back to `turn/start`; it clears stale processing/turn state when applicable, surfaces an error, and returns `steer_failed`.
