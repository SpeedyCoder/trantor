...
BETA_DESCRIPTION="${BETA_DESCRIPTION:-Trantor iOS beta build for external testing.}"
...
REVIEW_NOTES="${REVIEW_NOTES:-Trantor iOS beta build for external testing.}"
SKIP_BUILD=0
SKIP_SUBMIT=0
...
Options:
  ...
  --ipa <path>               IPA path (default: src-tauri/gen/apple/build/arm64/Trantor.ipa)
...
Examples:
  ...
  ./scripts/release_testflight_ios.sh --skip-build --ipa "src-tauri/gen/apple/build/arm64/Trantor.ipa"
USAGE
}
...
if [[ -z "$BUNDLE_ID" ]]; then
  BUNDLE_ID="com.dimillian.trantor.ios"
fi
...
if [[ -z "$IPA_PATH" ]]; then
  IPA_PATH="src-tauri/gen/apple/build/arm64/Trantor.ipa"
fi
