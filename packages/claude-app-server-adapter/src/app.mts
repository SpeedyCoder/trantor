...
import { normalizeInputItems, parsePrompt } from "./input.mjs";
import { createJsonRpcServer } from "./jsonRpc.mjs";
import {
...
import { notify, sendError, sendResult } from "./protocol.mjs";
import { runClaudeTurn } from "./sdk.mjs";
...
type AppServerArgs = {
  ...
  send: (payload: JsonRpcNotification | { id: unknown; result?: unknown; error?: unknown }) => void;
...
export async function createAppServer({
  ...
  async function runTurn(params: Record<string, unknown>) {
    ...
    send(notify("thread/status/changed", { threadId: thread.id, status: "running" }));
    send(notify("item/started", { threadId: thread.id, turnId, item: userItem }));
    send(notify("item/completed", { threadId: thread.id, turnId, item: userItem }));
    send(
      notify("item/started", {
        threadId: thread.id,
        turnId,
    ...
    try {
      ...
      if (result.text || !result.aborted) {
        const assistantItem = buildAssistantMessageItem(agentItemId, result.text, turnId);
      ...
      send(notify("thread/status/changed", { threadId: thread.id, status: "idle" }));
  ...
  const handlers: Record<string, MethodHandler> = {
    ...
    async "thread/start"(params) {
      ...
      await repository.save(thread);
      send(notify("thread/started", { threadId: thread.id, thread: summarizeThread(thread) }));
    ...
    async "thread/fork"(params) {
      ...
      send(notify("thread/started", { threadId: clone.id, thread: summarizeThread(clone) }));
    ...
    async "thread/name/set"(params) {
      ...
      thread.updatedAt = now();
      await repository.save(thread);
      send(notify("thread/name/updated", { threadId: thread.id, name: thread.name }));
      return { ok: true };
    },
    ...
    async handleRequest(request: JsonRpcRequest) {
      ...
      if (!handler) {
        if (Object.prototype.hasOwnProperty.call(request, "id")) {
          send(sendError(request.id ?? null, `Unsupported method: ${request.method}`));
        } else {
          send(notify("error", { message: `Unsupported method: ${request.method}` }));
        }
        return;
      }
