...
This fork includes a **proof-of-concept** daemon that runs Trantor's backend logic in a separate process (intended for WSL2/Linux), exposing a simple **line-delimited JSON-RPC** protocol over TCP.
...
cargo run --bin trantor_daemon -- \
  --listen 127.0.0.1:4732 \
  --data-dir "$HOME/.local/share/trantor-daemon" \
  --token "$TOKEN"
```
