...
Trantor is a Tauri app that orchestrates Codex agents across local workspaces.

- Frontend: React + Vite (`src/`)
- Backend app: Tauri Rust process (`src-tauri/src/lib.rs`)
- Backend daemon: JSON-RPC process (`src-tauri/src/bin/trantor_daemon.rs`)
...
4. Daemon RPC surface (`src-tauri/src/bin/trantor_daemon/rpc.rs` + `rpc/*`).
...
- Daemon entrypoint: `src-tauri/src/bin/trantor_daemon.rs`
- Daemon RPC router: `src-tauri/src/bin/trantor_daemon/rpc.rs`
...
- `src-tauri/src/bin/trantor_daemon/rpc.rs`
